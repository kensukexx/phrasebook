...
- **収録フレーズ**: 308件。1フレーズ＝日本語＋17カテゴリのいずれか＋12言語分の[発音表記, カタカナ読み]＋単語ごとの意味対応(gloss)＋解説(note)。308件全件に12言語分のgloss・解説がある（`tests/data-integrity.test.js`で検証、LANGSは同ファイル内に別途ハードコードされているため、言語を追加したらそちらの配列も更新すること）。
...
## ファイル構成
```
index.html          アプリ本体（UI・データ・ロジック全て、単一ファイル、約715KB）
