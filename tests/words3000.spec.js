...
test.describe('英単語3000（頻出英単語を頻度順に学ぶ独立モード）', () => {
  ...
  test('the tier selector is computed from the data (three 500-word tiers for the current 1200-word total)', async ({ page }) => {
    await page.goto('/index.html');
    await page.waitForSelector('#deck .ticket');
    await page.click('#toolsBtn');
    await page.click('#menuWords3000');
    ...
    await expect(page.locator('.w3k-card')).toHaveCount(200);
  });
