...
test.describe('英単語3000（頻出英単語を頻度順に学ぶ独立モード）', () => {
  ...
  test('the tier selector is computed from the data (five 500-word tiers for the current 2400-word total)', async ({ page }) => {
    await page.goto('/index.html');
    await page.waitForSelector('#deck .ticket');
    await page.click('#toolsBtn');
    await page.click('#menuWords3000');
    ...
    expect(options).toEqual(['1〜500語', '501〜1000語', '1001〜1500語', '1501〜2000語', '2001〜2500語']);
    ...
    await expect(page.locator('.w3k-card')).toHaveCount(400);
  });
